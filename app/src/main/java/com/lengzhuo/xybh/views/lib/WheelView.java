package com.lengzhuo.xybh.views.lib;
...
public class WheelView extends View {
    ...
    // 绘制几个条目，实际上第一项和最后一项Y轴压缩成0%了，所以可见的数目实际为3
    int itemsVisible = 5;
    ...
    private void remeasure() {//重新测量
        ...
        halfCircumference = (int) (itemHeight * (itemsVisible - 1))+20;
    ...
    protected void onDraw(Canvas canvas) {
        ...
        while (counter < itemsVisible) {
            ...
            // angle [-90°,90°]
            float angle = (float) (45D - (radian / Math.PI) * 90D);//item第一项,从90度开始，逐渐递减到 -90度
            ...
            if (angle >= 45F || angle <= -45F) {
                ...
                if (translateY <= firstLineY && maxTextHeight + translateY >= firstLineY) {
                    ...
                    canvas.restore();
                    canvas.save();
                    canvas.clipRect(0, firstLineY - translateY, measuredWidth, (int)
                            (itemHeight));
                    ...
                    canvas.scale(1.0F, (float) Math.sin(radian) * SCALECONTENT+0.1f);
                    //canvas.scale(1.0F, 0.9f);
                    canvas.drawText(contentText, drawOutContentStart, maxTextHeight, paintOuterText);
                    canvas.restore();
                }
